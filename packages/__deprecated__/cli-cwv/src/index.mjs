...
const token = get("GITHUB_TOKEN") || process.env.GITHUB_TOKEN;
if (!token) {
...
if (previewUrl) {
  ...
  urls = urls.map((u) =>
    u.includes("$BASE_URL") ? u.replaceAll("$BASE_URL", baseUrl) : u,
  );
  rc.ci.collect = {
    ...(rc.ci.collect || {}),
...
const code = await run(
  "npx",
  ["-y", "@lhci/cli@0.15.x", "autorun", "--config", tmpRcPath],
  { cwd: workspace },
);
if (code !== 0) {
